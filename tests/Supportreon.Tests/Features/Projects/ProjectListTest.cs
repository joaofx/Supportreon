...
namespace Supportreon.Tests.Features.Projects
{
    public class ProjectListTest : OneCaseFeatureTest
    {
        ...
        public override async Task Given()
        {
            ...
            await _.Save(_art, _food, _artProjects, _foodProjects, _closedProject);
        }

        [Test]
        public async Task Can_list_projects()
        {
            var result = await _.Send(new ProjectList.Query());
        ...
        {
            var result = await _.Send(new ProjectList.Query { Closed = true });
        ...
        {
            var result = await _.Send(new ProjectList.Query { Category = _art.Name });
        ...
        {
            var result = await _.Send(new ProjectList.Query { Search = _artProjects.At(0).Name });
